...
\begin{proof}
	...
	$$
	Assume for the moment that $a_s=a_x$, and let $\sigma_1,..\sigma_8$ the first singular values of $a_x^{-1}-a_x^{-t}$. In particular, 
	$
	\|a_x^{-1}-a_x^{-t}\|\leq \sum_{i_1}^8\sigma_i.
	$
	Consider  two grid functions $U_{i,j,k}, V_{i,j,k}$
	such that for any $j,k$ $V_{,j,k}$ and $U_{+1/2,j,k},U_{-1/2,j,k}$ are orthonormal eigenvectors corresponding to eigenvalue $\lambda_{} $ and of unit norm with respect to $x$ coordinate. Then  
	\begin{align*}
		& (<a_x^{-1}-a_x^{-t}>D_x U,V)_{\Gwh^\e}
\leq \\&
	h^2\sum_k\sum_j\sum_i \sum_{l} (a_{il}^{-1}-a_{il}^{-t})\frac{U(l+n/2,j,k)-U(l-n/2,j,k)}{h}V_{i,j,k}\\&
		=
	h^2\sum_j\sum_k\lambda^{i,j} \leq \|a^{-1}-a^{-t}\|=\\&
	\|a^{-1}-a^{-t}\|\sum_{i,j,k}(U(l+n/2,j,k)-U(l-n/2,j,k))V_{i,j,k}\leq 
		2\|a^{-1}-a^{-t}\|\|U\| \|V\|.
	 	\end{align*}
 Since $\mathrm{rank}$ of $a^{-1}-a^{-t}$ is constant, $C$, the general case we will get that $$(<a_x^{-1}-a_x^{-t}>D_x U,V)_{\Gwh^\e}\leq2 C\|a^{-1}-a^{-t}\|\|U\|\|V\|.$$
\end{proof}

		Next, we extend the operator $P_2$ and $P_1$ symmetrically  to $\Gw^{\e}$ and 
...
%\begin{cor}
%	Let 
%	$$\mathcal{\widetilde{E}}^{n}:=\mathcal{E}^n-\Delta \tau 	\frac{(A\e_h^{n},\h_h^{n+1/2})}{2}
%	-
%	\Delta \tau^2\frac{(A\e_h^{n},\frac{P_2}{P_1}a^{-1}\e_h^{n})}{2}
%	$$
%	Then $\mathcal{\widetilde{E}}^{n+1}\leq \mathcal{\widetilde{E}}^{n}$.
%\end{cor}
%\begin{lem}
%	\begin{itemize}
%		\item 
...
%	\begin{align*}
%	{\mathcal E}^{n}:=\|\e^{n}\|^{2}_{\e}+\|\h^{n+1/2}\|^{2}_{\h}
%	-\Delta \tau \left (\widetilde{ \curl}_h \e_h^{n},\h^{n+1/2}\right )_{\h}
%\end{align*}
%\end{rem}
