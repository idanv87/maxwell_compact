...
\begin{document}
	...
	\end{align}
	with initial conditions given for $\vec{D}$ and $\vec{E}$ at $t=0$.
...
\end{enumerate}
	...
	$$
	The equations all have the same form. So to discretize the equations to fourth order accuracy in both space and time
	we need only consider the equation for $D_z$. All the others will follow the same pattern.
...
In the following sections we will exploit fourth order compact schemes for solving elliptic equations of the form $-\Delta \phi+k^2\phi=F$ in $\Gw^{E_s}$ or $\Gw^{H_s}$, $s=x,y,z$.
...
+k^2\phi=F.
$$
The latter scheme requires boundary conditions for the inhomogeneous term and for $\phi$. We denote these boundary conditions as 
$B_{NH}$ and $B_{C}$ respectively.

	\subsection{Operators}
	For $s=x,y,z$, let $a_s^{-1}$ be the fourth order first-order derivative  compact operator given by Appendix \ref{appendix}.
	\begin{enumerate}
		\item $\frac{\Delta \tau }{h}=r$
		\item $\delta_x U(x_i,y_j,z_k):=a_x^{-1}\circ\frac{U(x_i+h/2,y_j,z_k)-U(x_i-h/2,y_j,z_k)}{h}$
		\item $\delta_y U(x_i,y_j,z_k):=a_y^{-1}\circ\frac{U(x_i,y_j+h/2,z_k)-U(x_i,y_j-h/2,z_k)}{h}$
		\item 	$\delta_z U(x_i,y_j,z_k):=a_z^{-1}\circ\frac{U(x_i,y_j,z_k+h/2)-U(x_i,y_j,z_k-h/2)}{h}$
		\item $
		L_{\tau}^h(\phi):=-
		\left (
		\frac{D_{xx}\phi}{1+\frac{h^2}{12}D_{xx}}+\frac{D_{yy}\phi}{1+\frac{h^2}{12}D_{yy}}
	...
	Moreover, we define 
	$$
	P({\bf J}):=Z\frac{24}{\Delta \tau^2}\left ( -{\bf J}^{n+1/2}-\nabla div({\bf J}^{n+1/2})+\frac{\partial^2{\bf J}^{n+1/2}}{\partial t^2}\right ).
	$$

	\section{The scheme}
	Let $\bf{E}^n, \bf{H}^{n+1/2}$ be given:\\[2mm]
	{\bf step 1}:\\[2mm]
	$$
	\begin{pmatrix}
		E_x^{n+1}\\
	...
	\end{pmatrix}=
	\begin{pmatrix}
		H_x^{n+1/2}+\Delta \tau H_x^{*}\\
		H_y^{n+1/2}+\Delta \tau H_y^{*}\\
		H_y^{n+1/2}+\Delta \tau H_y^{*}
	\end{pmatrix}
	$$
	where 
	...
	$B_{NH}$ equals to the non-homogeneous term which is defined on $\Gw^{\bf{H}}$ including the boundary.
	For $B_{C}$ we use Taylor expansion
	$$
	\frac{\bf{H}^{n+3/2}-\bf{H}^{n+1/2}}{\Delta \tau}=
...
F_{x,i+1/2,j,k}G_{x,i+1/2,j,k}+
F_{y,i,j+1/2,k}G_{y,i,j+1/2,k}+F_{z,i,j,k+1/2}G_{z,i,j,k+1/2}
$$
\begin{align*}
	&
	(U,V)_{\bf{H}}:=
	h^3\sum_{i=0}^{N-1}\sum_{j=0}^{N-1}\sum_{k=0}^{N-1}
	F_{x,i,j+1/2,k+1/2}G_{x,i,j+1/2,k+1/2}+\\&
	F_{y,i+1/2,j,k+1/2}G_{y,i+1/2,j,k+1/2}+F_{z,i+1/2,j+1/2,k}G_{z,i+1/2,j+1/2,k}
\end{align*}
...
\end{lem}
	...
	\begin{rem}
		All terms  	in equation \eqref{eq:system}  involving $\vec{J}$
		and its derivatives
		are known analytically, and therefore we choose to demonstrate our results by assuming without loss of generality  that $\vec{J}=0$.
	\end{rem}
	...
	\begin{equation}\label{eq:mu_eps_constant_lap}
		...
		\right)^{n+1/2}+\frac{c^2 \Delta t^2}{24}\left (
		-
	...
	\delta_{\tau} \vec{H}=\frac{\vec{H}^{n+1/2}-\vec{H}^{n+3/2}}{\Delta \tau}.
	$$
	...
	\begin{equation}\label{eq:system}
		...
		\end{pmatrix}
		\begin{pmatrix}
			\nabla \times \vec{H}^{n+1/2} -\vec{J}^{n+1/2}\\
			-\nabla \times \vec{E}^{n+1}
		\end{pmatrix}+
		\frac{\Delta \tau ^2}{24}
		\begin{pmatrix}
			\left(-\nabla div(\vec{J})+
			\frac{\partial^2 \vec{J}}{\partial t^2}\right)^{n+1/2} \\
			0
		\end{pmatrix}.
	\end{equation}
	...
	%	(-\frac{\partial E_y}{\partial x} + \frac{\partial E_x}{\partial y} )^{n+1/2}
	...
	\section{Transverse magnetic waves  in $[0,1]^3$}\label{sec:TE}
	...
	\end{enumerate}
	and the scheme \eqref{eq:system} reads as the following consecutive steps.
	\begin{enumerate}
		\item  $ E_z^{n+1}=E_z^n(i,j)+Z\Delta \tau \cdot L^{-1}_{\tau}\left (\frac{\partial H_y}{\partial x}-
		\frac{\partial H_x}{\partial y}
		\right)^{n+1/2}$\\[1mm]
		\item $ H_x^{n+3/2}=H_x^{n+1/2}+\frac{\Delta \tau}{Z} \cdot  L^{-1}_{\tau}\left (-
		\frac{\partial E_z}{\partial y}
		\right)^{n+1} $\\[1mm]
		\item $ H_y^{n+3/2}=H_y^{n+1/2}+\frac{\Delta \tau}{Z} \cdot  L_{\tau}^{-1}\left (
	...
	$\left (\frac{\partial H_y}{\partial x}-\frac{\partial H_x}{\partial y}\right )^{n+1/2}$ can be estimated using Pad\'e approximation (see  Appendix \ref{appendix}) at all points 
	$(x_i,y_j)$
	which do not belong to $\partial [0,1]^2$.  However, at these boundary points, the equations readily imply that  
	$\left (\frac{\partial H_y}{\partial x}-\frac{\partial H_x}{\partial y}\right )^{n+1/2}=0$ on $\partial [0,1]^2$.\\[1mm]
	{\bf Step 2} (See Figure \ref{fig:Hx}): 
	The numerical domain: $\{(x_i,y_{j+1/2})\}_{i\in [N],j\in [N-1]}$; 
	...
	boundary conditions:
	we  estimate 
	$\frac{H_x^{n+3/2}-H_x^{n+1/2}}{\Delta \tau}$ on the boundary as follows.
	The equation  
	$$
	...
	$$\frac{H_x^{n+3/2}-H_x^{n+1/2}}{\Delta \tau}=0$$ if $x=0$ or $x=1$.
	On the remaining part of the boundary ($y=0,1$)  we use Taylor series:
	$$
	\frac{	H_x^{n+3/2}-H_x^{n+1/2}}{\Delta \tau}=
	...
	{\bf Step 3} : \\The numerical domain including the boundary: $\{(x_{i+1/2},y_{j})\}_{i\in [N-1],j\in [N]}$; the boundary terms and the boundary conditions are treated by a similar way to step 2.
	...
	%	\begin{table} [h!]
		...
		%	\begin{table} [h!]
			...
			$x_{1/2},..,x_{N-3/2}$. This can be done  by solving the system of equations
			\begin{equation}
				\frac{1}{24}
				\begin{pmatrix}
					26     & -5     & 4      & -1     & 0      & \dots  & 0      \\
			...
			\begin{align*}
				\left (\frac{\partial D_z}{\partial t} \right)^{n+1/2} &= \frac{D_z^{n+1} - D_z^{n}}{\Delta t} - \frac{(\Delta t^2)}{24} \frac{\partial D^3_z}{\partial t^3} + O(\Delta t)^4 \\
			...
			We then get
			\begin{align*}
				\left (\frac{\partial D_z}{\partial t} \right)^{n+1/2}  &= \delta_t D_z - \frac{\Delta t^2}{24} \partial_{tt} \left(\frac{\partial {B_y \over \mu}}{\partial x }
			...
			\begin{align*}
				...
				\frac{\partial H_x}{\partial y}-J_z
				\right)^{n+1/2}=\\&
				\frac{\partial D_z^{n+1/2}}{\partial t} =
			...
			The term
			$$
			div\left(\mu^{-1}\frac{\partial^2\vec{E}}{\partial t \partial z}\right )^{n+1/2}
			=div\left(
			...
			\left ( \frac{\partial H_y}{\partial x}-
			\frac{\partial H_x}{\partial y}-J_z
			\right)^{n+1/2}+\frac{\Delta t^2}{24}\left (
			div\left(
			\mu^{-1}\frac{\partial }{ \partial z}\frac{1}{\varepsilon}(
			\nabla \times \vec{H} -\vec{J}
			)
			...
			We assume that $E^0$ and $H^{1/2}$ are given to fourth order.
			For $H^{1/2}$ this can be done using  standard Taylor approximation.
			Next,
			$$
			\frac{\vec{E}^{1}-\vec{E}^{0}}{\Delta t}=\frac{\partial \vec{E}^{1/2}}{\partial t}+
			\Delta t^2\frac{\partial^3 \vec{E}^{1/2}}{\partial t^3}+O(\Delta t^4).
			$$
			The equation for $\vec{E}$ imply that 
			$$
			\frac{\partial \vec{E}^{1/2}}{\partial t}=\varepsilon^{-1}\cdot \nabla \times \vec{H}^{1/2}.
			$$
			...
			\begin{align*}
				&
				\frac{\partial^3 \vec{E}^{1/2}}{\partial t^3} =
				\varepsilon^{-1}\nabla \times  \frac{\partial^2 H^{1/2}}{\partial t^2}=
				\\&
				-\varepsilon^{-1}\nabla\times \mu^{-1}
				\nabla \times \frac{\partial \vec{E}^{1/2}}{\partial t}=
				-\varepsilon^{-1}\nabla\times \mu^{-1} \nabla \times
				\varepsilon^{-1}\nabla\times \vec{H}^{1/2}.
			\end{align*}
			The term $ \nabla\times \vec{H}^{1/2}$ is known to fourth order and therefore it is sufficient to approximate its  second-derivatives to second order. Since 
			$\frac{\partial^3 \vec{E}^{1/2}}{\partial t^3}$ is multiplied by $\Delta t^2$ we have accomplished our estimation to order $h^4+\Delta t^2h^2$.
