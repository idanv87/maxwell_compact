...
By letting  $\Delta \tau=c\Delta t$,  $Z=\sqrt{\mu/\varepsilon}$, we look for a numerical scheme for the following problem.
%$$
...
$	\delta_{\tau} \bf{E}$, and $\delta_{\tau} \bf{H}$.
	...
	\begin{defi}
		...
		Let $$L^I:=
		\left (\frac{P_1^{I}}{P_2^I}\right)\phi+\left (\frac{P_1^I-P_2^I}{P_2^I}\right)B^I
		$$
	...
	{\bf step 1}: solve  $\e^{n+1}_h$ in $\intr\Gw^{\e}$: \\[2mm]
	$$
	\begin{pmatrix}
		E_x^{n+1}\\
		E_y^{n+1}\\
...
Consider the equation based  Taylor expansion
	...
	$$
	Hence our choice for boundary conditions is indeed consistent with the condition 
	$B_\phi=B_F$. Therefore,
	$$\begin{pmatrix}
			B^{H_x}\\ B^{H_y}\\ B^{H_z}
	\end{pmatrix}
=
	...
	\end{pmatrix}
	$$
	evaluated in $\partial\Gw^{\h}$. This boundary condition is of order $\Delta \tau^2$.
...
\end{align*}
The induced norms will be denoted by $\|\cdot \|_{\e},\|\cdot \|_{\h}$.
...
\end{pmatrix}.
$$
\begin{corollary}(cf. \cite[(37)]{sakka} for the case $a=1$)\label{lem:sym}
	Assume that $a=a^{\mathrm{sym}}$. Then
