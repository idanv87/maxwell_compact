...
\begin{document}
	...
	Let $\e,\h,\bf{D},\bf{B},\bf{J}$ be (real-valued) vector fields defined in  $[0,\infty)\times\Gw$ and take values in $\R^3$.
...
\begin{enumerate}
	...
	\item  $\Gw=[0,1]^3$, $\vec{n} \times {\bf E}=0$ on $\partial \Gw$ (\cite[Section 8]{rolf}).
\end{enumerate}
...
%$$
	...
	\label{eq:maxwellv}
	&\notag
\frac{\partial {\bf E}}{\partial \tau }=Z(\nabla \times \bf{H}-{\bf J}) \quad \mathrm{in} \quad  (0,\infty)\times \Gw,\\&\notag
 \frac{\partial {\bf H}}{\partial \tau }=-\frac{1}{Z}\nabla \times {\bf E} \quad \mathrm{in} \quad  (0,\infty)\times \Gw,\\&\notag
...
%		$D_{ss}\phi=\frac{\phi(s+h)-2\phi(s)+\phi(s-h)}{h^2}$ is standard symmetric second derivative operator for $s=x,y,z$ with Dirichlet boundary conditions.
   ...
   In the following sections we will exploit fourth-order compact schemes for solving elliptic equations of the form $-\Delta \phi+k^2\phi=F$,$k>0$.
Let 
$$
...
$$
P_1\phi+P_1B_\phi=P_2\phi+P_2B_{F}
$$
where $P_1,P_2$ are symmetric operators.
If further $B_{\phi}=B_{F}=B$, then 
\begin{equation}\label{eq:explain}
\left (\frac{P_1}{P_2}\right)\phi+\left (\frac{P_1-P_2}{P_2}\right)B=F.
\end{equation}
...
$	\delta_{\tau} \bf{E}$, and $\delta_{\tau} \bf{H}$.
	...
	\begin{defi}
		...
		let $$P_1^{I}, P_2^{I}:\intr\Gw^{I}\to \intr\Gw^{I}$$ defined as 
		$$
		P_1^{I}:=-\left (\Delta_h+\frac{h^2}{6}\Delta_h^2\right)+\frac{24}{\Delta \tau^2}\left (
		 1+\frac{h^2}{12}\Delta_h^2,
		\right)
		$$
		$$
			P_2^{I}:=\frac{24}{\Delta \tau^2}\left (
		1+\frac{h^2}{12}\Delta_h^2.
		\right)
		$$	
		let $B^I$ be $I$ evaluated in $\partial \Gw^{I}$.
		Let $$L^I:=
		\left (\frac{P_1^{I}}{P_2^I}\right)+\left (\frac{P_1^I-P_2^I}{P_2^I}\right)B^I
		$$
		
	\end{defi}
\begin{lem}\label{lem:bottom_spectrum}
The bottom of the spectrum of $	\frac{P_1^{I}}{P_2^I} $
is greater than $1$, and the top is less then $1+\frac{r^2}{6}+O(h^2)$.The top of the spectrum of 
$\frac{P_1^I-P_2^I}{P_2^I}$ is less then  $\frac{\Delta \tau^2}{24}\sigma_{\max} (-\Delta_h$).
\end{lem}
	\begin{rem}		
...
$$	\left.{\frac{x}{\frac{a+b}{c}}}\right|_{\;0}^{\;} $$
...
\left [ \left.
	...
	$$
	Extend $$	\left.\begin{pmatrix}
		E_x^{n+1}\\
		E_y^{n+1}\\
		E_z^{n+1}
	\end{pmatrix}\right|_{\; \intr{\Gw^{\e}}}^{\;}$$
to $\Gw^\e$ using the boundary conditions $B^{\e}=0$ and derive 
$\e_h^{n+1}$ in $\Gw^\e$.
	\\[1mm]
	...
	\end{pmatrix}
	=
	-\frac{1}{Z}\left[ \left.	\begin{pmatrix}
		0& -\delta_z & \delta _y\\
		\delta_z&0&-\delta_x\\
		-\delta_y&\delta_x&0\\
	\end{pmatrix}
...
\left[ \left.
	...
	$$
		...
		J_z^{n+3/2}
	\end{pmatrix}\right|_{\; \intr{\Gw^{\e}}}^{\;}$$
	to $\Gw^\h$ using the boundary conditions. 
...
Consider the equation based  Taylor expansion
 ...
 indeed consistent with the condition 
	$B_\phi=B_F$ (cf. \eqref{eq:explain}).
...
\begin{lemma}(cf. \cite[(37)]{sakka} for the case $a=1$)\label{lem:sym}
	Assume that $a=a^{\mathrm{sym}}$, $\e_h\in \Gw^\e$ and $\h_h\in \Gw^\h$,
	$\bf{E}_h$ vanishing on $\partial \Gw^{\bf{E}}$.
	\begin{itemize}
		\item  
		$
		(\mathbf{\curl_h}(\bf{E}_h),\bf{H}_h)_{\Gw^\h}=
		(\mathbf{\curl}'(\bf{H}_h),\bf{E}_h)_{\Gw^\e}.
		$\\[1mm]
		\item $\|a_s\|\geq \frac{5}{6}$\\[1mm]
		\item $(\curl _h\e_h,\h_h)_{\Gw^\h}\leq\|a_s^{-1}\|\cdot  \frac{2\sqrt{3}}{h}\|\e_h\|_{\Gw^\e}
		\|\h_h\|_{\Gw^\h}
		$.
...
\begin{defi}
	...
	&
	\mathcal{E}^{n}:=
	(\frac{P_1}{P_2}\e_h^{n},\e_h^{n})_{\intr{\Gw^\e}}
	-\Delta \tau (\curl_h\h_h^{n+1/2},\e_h^n)+\\&
	(\frac{P_1}{P_2}\h_h^{n+1/2},\h_h^{n+1/2})_{\intr{\Gw^\h}}+
	(\h_h^{n+1/2},\h_h^{n+1/2})_{\partial \Gw^{\h}}.
\end{align*}
\end{defi}
\begin{lemma}\label{lem:coer}
	Assume for simplicity that $\J=0$ and $Z=1$.
	Assume that $a_s=a_s^{\mathrm{sym}}$ for all $s=x,y,z$.
%If 	$$\frac{\Delta \tau}{h}\leq \frac{5}{6\sqrt{3}}$$
Then 
\begin{itemize}
		\item $ {\mathcal E}^{n}\geq 
	\|\e_h^{n}\|^{2}_{\Gw^\h}+\|\h_h^{n+1/2}\|^{2}_{\Gw^\h}-
	\frac{6\sqrt{3}r}{5}\left ( \|\e_h^{n}\|_{\Gw^\e}^2+\|\h_h^{n+1/2}\|_{\Gw^\h}^2 \right).$
	
			\item $ {\mathcal E}^{n}\leq 
	\|\e_h^{n}\|^{2}_{\Gw^\h}+\|\h_h^{n+1/2}\|^{2}_{\Gw^\h}-
	\frac{6\sqrt{3}r}{5}\left ( \|\e_h^{n}\|_{\Gw^\e}^2+\|\h_h^{n+1/2}\|_{\Gw^\h}^2 \right).$
	
	\item $	{\mathcal E}^{n+1}=	{\mathcal E}^{n}-
...
\end{lemma}
\begin{proof}
	The first assertion follows immediately by Lemma \ref{lem:sym} Young's inequality and \cite[Theorem 4.1]{sakka}.
	
	Next, we take inner products of the scheme with averaged values
\begin{align*}
	&
	(\frac{P_1}{P_2}(\e^{n+1}_h-\e^{n}_h),\e^{n+1}_h-\e^{n}_h)_{\intr{\Gw^\e}}+
	(\e^{n+1}_h-\e^{n}_h,\e^{n+1}_h-\e^{n}_h)_{\partial{\Gw^\e}}
	\\&
	=
	\Delta \tau(\curl_h\h_h^{n+1/2},
...
\begin{align*}
	&
		(\frac{P_1}{P_2}(\h^{n+3/2}_h-\h^{n+1/2}_h),\h^{n+3/2}_h+\h^{n+1/2}_h)_{\intr{\Gw^\h}}+\\&
	(\frac{P_1-P_2}{P_2}B(\h^{n+3/2}_h-\h^{n+1/2}_h),\h^{n+3/2}_h+\h^{n+1/2}_h)_{\intr{\Gw^\h}}
	+
	(\h^{n+3/2}_h-\h^{n+1/2}_h,\h^{n+3/2}_h+\h^{n+1/2}_h)_{\partial{\Gw^\h}}
	\\&= 
		-\Delta \tau(\curl_h\e_h^{n+1},
	\h^{n+3/2}_h+\h^{n+1/2}_h)_{\intr{{\Gw^\h}}}+
	(\h^{n+3/2}_h-\h^{n+1/2}_h,\h^{n+3/2}_h+\h^{n+1/2}_h)_{\partial{\Gw^\h}}=\\&
	-\Delta \tau(\curl_h\e_h^{n+1},
	\h^{n+3/2}_h+\h^{n+1/2}_h)_{{{\Gw^\h}}},
\end{align*}
where the last inequality follows by Remark \ref{rem:expB}.
Lemma \ref{lem:sym} and summation imply 
\begin{align*}
	&
...
%	
	...
	%	\begin{table} [h!]
		...
		\begin{equation}\label{eq:lhs2}
			...
			\end{pmatrix}.
		\end{equation}
			The matrix operator in the left hand side of \eqref{eq:lhs2} will be denoted by $a^{\mathrm{sym}}$.
